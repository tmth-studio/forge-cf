...
> ⛔ **VA-147 — the gateway partner is classed by which side of the venture's own effect it sits on, and priced against the pool it stands to lose (Head of R&D, RD-032, 17 September 2026; object-after 19 September 2026 14:00 BST unless Tom objects).** Before naming the partner, SR2 lists every candidate present in the customer's routine in two classes: **loss side** (the architecture takes a profit pool from them) and **gain side** (the venture pays them a fee or they earn a credit). Where both stand in the routine, R7 prefers the loss side, on two things the run shows: size (the loss-side case is bounded by the pool at risk R2 printed; the gain-side case by the fee) and durability (surplus at launch and at the at-scale count; a gain-side surplus that turns negative at scale is a launch partner, not a gateway partner). A loss-side actor's best alternative is to lose the pool, never the status quo (VA-127 corrected). A gain-side selection with a loss-side candidate present carries a stated reason from the admissible list in SR2. A stated absence of any loss-side party is admissible on falsifiable tests (VA-92). **What fails R7 is not asking.** Regression R-C7b and R-C7c. Canon: stakeholder identification includes adversely affected parties (15288 §6.4.2; NASA SE Handbook Rev 2 §4.1.1.2.1); the preference itself has no SE analogue. Literature: Anthony, Gilbert and Johnson (2017); Christensen and Raynor (2003) on asymmetric motivation.
...
**Whose routine — declare it first (VA-154, RD-038 (Head of R&D, 21 Sep 2026, object-after 23 Sep 12:30)).** Open SR1 with one line: *customer of record: [role]; the routine positioned in: [its solution-seeking routine for this outcome]*. Where the venture has more than one customer of record (a payer and a site; a claimant and a funder), **R7 runs once per customer of record**, each with its own SR1–SR5 and verdict; one run on "the customer" is refused. And where the C10 run names a supplier as binding at launch, a **shelf check** runs for that supplier: where does it come to buy things like us, and is the venture present there? The theory is the same on both sides (physical availability — Sharp 2010; Romaniuk and Sharp 2016); one R7 of record applied it to the end user while the site and the funder, whose doors bind, were never mapped (Tom, 17 Sep 2026). `[evidence: VA-154]`
...
**The two-class enumeration (VA-147, 17 September 2026) — run it before the partner is named.** Presence is necessary and not sufficient. List every candidate that is present in the routine at the moment of need, and for each state the sign of the architecture's effect on that candidate's own book:

| Class | Test | What its case is bounded by |
|---|---|---|
| **Loss side** | the architecture takes a profit pool from it — in whole or in part, now or as the venture scales | the pool at risk, from R2's `POOL AT RISK — R2` block; where R2 missed the party, add the row and size it |
| **Gain side** | the venture pays it, or it earns a fee or credit from carrying the product | the fee, against the next-best use of the same capacity |

*Two classes, not three (aligned 21 September 2026 to VA-147 as ratified, on the Head of Verification's note).* A party whose book the architecture leaves unchanged and whom the venture would not pay has no business case and is not a candidate carrier: record it in the enumeration with sign 0 and move on. If the venture would pay it to carry the product, it is gain side.

**Preference.** Where a loss-side candidate and a gain-side candidate are both present, prefer the loss side. State the two things that carry the preference: the size of each case (pool against fee) and its durability (surplus at launch and at the at-scale count, with the mechanism if it falls). A loss-side party keeps a pool by carrying the product; its case grows as the venture grows. A gain-side party's fee competes with its own core margin the moment the product substitutes for that core; its case shrinks as the venture grows (asymmetric motivation — Christensen and Raynor 2003).

**A gain-side selection is admissible with one of these reasons, each with its arithmetic or evidence:** no loss-side party stands in the routine at the moment of need · the loss-side party fails `operational_fit` · the loss-side party is barred by regulation or by a perimeter rule the venture holds · the pool at risk per incumbent is below that incumbent's cost of carrying the product · the loss-side surplus at scale is lower than the gain-side surplus at scale. A selection with no reason fails the logical check.

**A stated absence is admissible.** Where the architecture takes no pool from anyone present in the routine, say so and name the test that would show otherwise (VA-92's third answer).

**Third presence disposition — a class present collectively (VA-155, RD-038 (Head of R&D, 21 Sep 2026, object-after 23 Sep 12:30)).** Presence has three dispositions, not two: *one party present in all routines* · *no party present, absence stated* · **a class present collectively, no member present in all** — activated by one product across its seats, with the seat named at which each member sits in the routine. An aggregate incumbent (professional bodies, platforms, a bank's relationship managers) is the common case for an organisational customer; naming one member as "the" gateway when no member is in every routine fails SR2.

**The partner's form is a named field, read before its count and its hours (VA-165, RD-042 (Head of R&D, 21 Sep 2026, object-after 23 Sep 17:00 BST)).** State `Form:` — *single firm* or *aggregate* (a platform, a consolidator, a franchisor, a large regional firm, a trade body) — before the partner count and before any hours figure is read. Where the customer is an organisation and a single firm is chosen, say why the aggregate form was not. Then state `Hours per close read for:` — the form the hours belong to and their source. An aggregate sale is an organisational decision over months; read its hours for that form, never from the single-firm approach figure. A count in one form and hours in the other is a named check not run (VA-89). The operating model's acquisition activity for the partner carries the same form label. *Instance: a record counted aggregate incumbents and read the single-firm hours figure for the close — `[evidence: VA-165]`.* The 6b check is PA-05 and PA-11 in `partnerships-standard.md`.

**The loss-side party as payer.** Where the loss-side party subscribes, channels its pool and takes a share of the venture's distributable cash, it is the R2 customer and R2 runs on it (Section 7's pricing note). Its key monetizable cost is then the pool at risk. As a carrier only, it keeps VA-99's five fields and the pool enters as the content of `earns` and `beats_next_best`.
...
GATEWAY PARTNER
  ...
  Form (VA-165): [single firm / aggregate — platform, consolidator, franchisor, large regional firm, trade body · if single firm for an organisational customer, why not aggregate]
  Hours per close read for (VA-165): [the form the hours belong to · the figure · its source — never the single-firm approach figure for an aggregate sale]
  ...
  Class (VA-147): [loss side / gain side — the sign of the architecture's effect on its own book]
  Loss-side candidates present: [list, each with its pool at risk from R2's block · or "none — [the test that would show otherwise]"]
  Reason if gain side selected: [one of the admissible reasons, with its arithmetic · or "n/a — loss side selected"]
  Surplus at launch / at scale: [two figures with the mechanism if the second is lower]
...
**VA-147 corrects the comparator (17 September 2026).** "Best alternative" is not the status quo for an actor whose pool the architecture takes. For that actor the alternative is the world in which the architecture exists and the actor does not carry it — the pool lost to the venture or to an attacker using the same architecture. Price `beats_next_best` against that. A retained share of the pool is a gain in `earns`. The VA-127 table gains two columns: **effect on own book** (takes · neutral · adds) and **surplus at scale** beside surplus at launch. A loss-side actor rejected on "negative business case — the venture removes its fee" has been priced against the status quo, and that row fails.
...
**The ceiling is the R2 key monetizable cost of whoever pays.** R2 sets the value ceiling; R7 does not set a second one. Corrected 7 September 2026 — see the note below.

**Where the gateway partner is also the payer, the gateway partner IS the customer for R2, and R2 is run on them.** Their ceiling is then the incremental value created per unit for them — not their current cost of doing the equivalent function in-house, which is a substitution floor and not a value ceiling. That distinction is the real content of this principle and it is unchanged: the ceiling is what a unit of the product is worth to the payer, measured as the incremental value it enables.

**Where the gateway partner is not the payer, R7 sets no ceiling at all.** The ceiling stays the paying customer's R2 key monetizable cost, and a gateway partner's incremental value is an activation argument, not a price.

> **⚠ Why this was corrected (NCR-034 — the finding that this file defined the ceiling two ways, two lines apart).** This block previously read "the ceiling is the incremental value created per unit for the gateway partner" without the payer condition, while the margin block below reads "WTP_low = R2 KMC / price ceiling". A venture whose gateway partner is not its customer took two different ceilings from adjacent lines of one instruction. Every margin of safety is read on the ceiling, so the gate figure depended on which line the run followed. The framework's own layer assignment settles it: R2 is the value ceiling, what customers pay. Raised by the Head of Verification from the Head of R&D's leakage sweep; the substance of the principle is preserved and only its reach is narrowed. **This is a method file — if the Head of R&D reaches a different resolution, theirs is the one that stands.**
...
**Count line — required (VA-155, RD-038 (Head of R&D, 21 Sep 2026, object-after 23 Sep 12:30)).** Print: *the count the gateway route carries at central: [n] · the at-scale count: [n] · the PCO screen: [n] · the objective: [n] · partners active (G15): [n], gate [threshold], margin [x]*. G15 is a gate with a margin, not a description.

**The partner's return — fee in the floor or share of surplus (VA-156, RD-038).** Classify how the partner is paid: **fee-in-the-floor** (a cost line per unit, inside the floor the margin is read on) or **share-of-surplus** (a distribution of what is left above the floor). Print the unit margin under each reading and state the venture's own reading with its reason. The classification decides which side of the margin the partner sits on, and on one record it decided the legal form. `[evidence: VA-156]`
...
**For R7:** Use the pricing unit confirmed in Section 7. Cost floor from the updated AOM. **Price ceiling = the R2 key monetizable cost of whoever pays** — where the gateway partner is the payer that is their incremental value per unit, not their substitution cost; where the gateway partner is not the payer, use the paying customer's R2 ceiling unchanged (see the Section 7 principle, corrected 7 September 2026). If either the cost floor or the value ceiling is a red (unvalidated) input, the verdict is provisional: *"⚠ Provisional — pending [named input]. Would FAIL if [stated threshold]."* Do not record a clean PASS on red inputs.
...
| **two-class enumeration** — every present candidate classed loss side · gain side (a party with no case recorded at sign 0, not as a class); loss-side cases priced against the pool from R2's block; surplus at launch and at scale for the selected partner; a gain-side selection carries its stated reason | **VA-147** |
