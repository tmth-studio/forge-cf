...
**Stage gates, kill bands and the yes-threshold (VA-163, RD-041 (Head of R&D, 21 Sep 2026, object-after 23 Sep 12:30)).** Where the Scaling Strategy stages capital on a kill rule, two things are stated in the same file before any count is read. (1) **The kill band per kind of instance.** A first instance of a family (a trial) and a repeat of a proven one (a replica) are different kinds; each carries its own band, and the replica band is a named unmeasured input until it is measured. One band for every kind overstates capital several times over. `[evidence: VA-163]` (2) **The yes-threshold is read inside a pre-registered acquisition budget.** State the acquisition spend the stage may buy before the count is read; a count an acquisition line could buy is not a kill signal, and a threshold with no budget beside it is blank.
...
This governs the *rendered document*, not the method: sub-requirement definitions quoted from this skill stay verbatim, theory citations keep their original titles, and internal working notes may stay in normal register. Reference example: `04-Projects/TMTH_Venture_Studio/Education/education-venture-design-record-2026-08-17.html`.
...
|-------|-------------------|---------|
| CTM | `[venture]-ctm-at-C[N].html` | `acme-ctm-at-C3.html` |
| AOM | `[venture]-aom-at-C[N].html` | `acme-aom-at-C3.html` |
| Fin-Sim | `[venture]-fin-sim-at-C[N].html` | `acme-fin-sim-at-C3.html` |
