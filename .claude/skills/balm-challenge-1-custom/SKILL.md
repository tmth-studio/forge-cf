...
**Named-business entry — "I want to disrupt X" (Tom, 22 September 2026).** When the user's words name a business to disrupt, that business **is** the Sub-requirement 1 input and hard stop 1 is met by construction: the PCO is answered by existence (the line is paid for at scale — PCO Step 4, Path C). The run writes the PCO block itself from public sources — the line (the largest by revenue if a whole company is named), the job levelled per Step 2d, current buyers with count, price and volume, the mode (default 3) and in mode 3 the incumbent's roadmap form — each figure red until sourced — and proceeds to Sub-requirement 1 without a PCO conversation. Do not ask the user to confirm a PCO, to name a core functionality or to name an excluded population. The front door (`architect-custom`, Step 0) states the same rule.
...
- **Path C (named business — "disrupt X"):** how the named business delivers the job today — made, sold, delivered, paid for — described in full from public sources. The business's own line is the instance of the conventional form; the form is how the market delivers that job, of which the line is one case. No variant statement ("cheaper X", "digital X") before the CLO.
