...
> **🚪 BECOME OPENS ON FOUR FIXED STATES (Tom's ruling, 4 September 2026 — applies to every venture, and to the Forge core functionality).** Every CTM's Become phase starts with these four, in this order, before any venture-specific state: **(1) [Actor] who is unaware the product exists** · **(2) [Actor] who understands the venture can help them reach the outcome by removing their KMC — roughly how it works, and where to get it** · **(3) [Actor] who recalls the product at a category entry point** · **(4) [Actor] who has found the product and begun the purchase journey.** This is the most current understanding the studio holds of how customers actually buy. **Why the fixed opening earns its place:** state 3 is the *retrieval gap* — the venture failing to come to mind at the moment the buying need fires — which is architecturally distinct from the *conviction gap* every design instinctively draws. A CTM that opens on a customer already aware has assumed the retrieval problem away, and its Communications Product will be built entirely as sales with no advertising sub-type, which is the Ehrenberg-Bass failure. **Name the category entry points explicitly at state 3** — the specific moments the need fires — or the state is decorative. **A venture may deliberately skip one:** a launch that reaches customers by direct approach creates the entry point by arriving. That is recorded as a DELIBERATE ABSENCE with the date it is revisited, never as a state that does not exist. A product absent at states 1 to 3 with no such note is a finding. First applied 4 Sep 2026 to Calmly's transaction-site journey, which had no retrieval state and no retrieval product.

> **🛬 EVERY ACTOR TRACK OPENS AT ARRIVAL (VA-102, Head of R&D ruling, 7 September 2026 — applies to every venture).** The four fixed Become states above put arrival at the front of the CUSTOMER's journey. **They bind no other actor.** `user_tracks` and `partner_tracks` inherit the schema, the person-state register and the narrative rule, and inherit nothing about where a track opens — so a funder track opened on a funder already at the table and nothing objected. **Model key:** each track may carry an `arrival:` block beside `actor`, holding either `not_applicable: <reason>` or `deliberate_absence: <reason>` **plus** `substitute: <the activity that does the work instead>`. Without one, the track's first state must be a state in which the actor does not yet know the venture exists. Models dated on or after 7 September 2026 error; earlier models warn. **Why it earns its place:** the resource model is a faithful image of the journey model, so a track that starts mid-way produces a cost model that starts mid-way. Calmly costed *"sell the layer into a site"* at 60 selling-days per win and *"work an opportunity that does not close"* at three per win, because its customer journey opened on `unaware`; it costed nothing at all for finding a funder, because its funder track opened on `assessing`. **Clause 2 is the one that bites.** A deliberate absence removes a *product* and never removes the *work* — Calmly's record said the launch reaches sites *"by direct approach and warm introduction"* and Forge's said the gateway permission approach is *"Tom-executed"*, and neither activity had a line in any resource model. **An activity named in prose and performed by a person is in the operating model, or it is not in the venture; assigning it to the principal moves it to a resource nothing sizes.** **The rule forbids no answer** — "we have no acquisition activity, by design, and here is the mechanism that replaces it" passes, provided the mechanism is named and its cost is driven. Forge's customer side is the reference pass: the customer is delivered by a partner surface, acquisition is compute per acquired customer at £8–19, and the paid-channel corner is published as a failing stress row. **The abuse test:** arrival is not `not_applicable` for an actor whose participation the architecture asserts. If the design fails when that actor declines, the actor arrived somehow, and the model says how. Finding: `R_and_D/rd-015-actor-arrival-2026-09-07.md`.

> **🈳 EMPTY PHASES MUST BE DECLARED, NOT LEFT SILENT (RD-013, Head of R&D, 8 September 2026).** A phase may be *designed* and legitimately hold no transition — an actor who does nothing in that phase. Calmly's claimant pays nothing: zero at intake, zero on outcome, zero if they decline, and the payment reaches them from the funder in another phase. That is an architectural invariant, not an omission. The standard previously could not say so, and the only ways through were to invent a self-transition (routing around a check that was working) or to move a transition between phases (a design act). Both are wrong. **Declare it: `disposition: { not_applicable: "<reason>" }` or `{ not_designed_until: C<N> }`.** The checking script refuses an empty phase without one, warns on a reason too short to explain anything, and warns on a disposition left behind on a phase that has since gained transitions. **Distinct from `undesigned`, which means not yet designed — using it for a designed-but-empty phase records something untrue.** Same principle as the absent-products dispositions (VA-72): a flag that requires a reader is not a gate.
...
> **🔁 THE DERIVATION IS A LOOP, NOT A CHAIN (Tom's proposal, 4 September 2026, extended by the CEO the same day — applies to every venture).** Tom's statement: *"CTM necessitates 4D products which necessitates people in org which necessitates routines."* **Correct on direction, and two things complete it.**
>
> **(1) Routines have TWO parents, not one.** A product-only derivation produces the routines that make a product WORK. It cannot produce a routine whose trigger is a product FAILING or an outside party demanding something. Test case, Tom's own: sales and onboarding derive from products; **complaints does not — no component produces a complaint. A complaint is what an actor does when a component fails in a way they object to, so its parent is the JOURNEY STATE the actor was supposed to reach and did not.** The second arc — `CTM → journey states not reached → failure and obligation routines` — is where complaints, incidents, objections, audit, and a customer leaving all live. Both arcs start at the CTM, so the chain still begins where Tom put it.
>
> **(2) Routines come in three kinds, matching allocate · measure · hold.** **RUN** (make the product work — fails visibly, somebody is waiting) · **MANAGE** (set the level, review what came back, decide exceptions, plan capacity — fails slowly and invisibly; nothing breaks, the numbers drift) · **HOLD** (check nothing was traded away for performance — fails as an IMPROVEMENT in the breaching function's own numbers). A register built by product derivation alone produces only RUN. Calmly's had 18 run, 0 manage, 2 hold.
>
> **(3) THE LOOP CLOSES BACK ONTO THE COST MODEL, and this is the load the rule carries.** Routines consume resource · resource is what the ARM sizes · the ARM's drivers are what the margin is computed from. So the last link runs back to the first. **A ROUTINE IS WHERE A COST DRIVER BECOMES A PROMISE.** "One integration engineer per ~17 live sites" is a claim about the throughput of a site-onboarding routine; with no such routine the number has nowhere to live and nobody to answer for it. **Found at Calmly 4 Sep: 16 cost drivers with no owner and no test, computing a precise margin.** Every venture check: for each driver in the resource model, name the routine accountable for it. A driver with no routine is a number nobody was asked to hit.
>
> **(4) One routine every venture needs and none generates: RE-RUN THE OPERATING MODEL ON ANY RATIFIED ARCHITECTURE CHANGE, and half-yearly regardless.** Calmly's absence of it is what let the operating model sit on a superseded product set for ten days — roles for retired products surviving, 24 current components never allocated, two resourcing drivers sized on counterparties that had left. First register: `calmlyresolve-management-routines-register-2026-09-04.html` (MR-01 to MR-23).
