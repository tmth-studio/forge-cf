---
name: balm-challenge-8-custom
description: IVE R8 — Create a Switching Cost. Identifies the mechanism by which the venture accumulates value in the customer relationship that makes switching to an alternative progressively more costly. Produces a Lock-in Strategy that increases retention without artificial barriers.
---

# BALM R8 — Create a Switching Cost

**Purpose:** Design a mechanism that makes the product more valuable to the customer over time — and therefore harder to leave — as a structural property of the architecture. R8 does not impose barriers on the customer. It accumulates value that makes the alternative progressively worse by comparison. This is the first requirement of Function 3.

**When to run:** After R7 (Activate Gateway Partners) is complete and the CTM and AOM reflect the full F2 architecture. R8 begins Function 3. The switching cost mechanism must be consistent with what was established during F2 — in particular, it often builds directly on the activation mechanism from R5.

**Output:** Accumulation Mechanism, Switching Cost Trajectory, Lock-in Strategy. Together these constitute the R8 component of the new Business Form Factor.

**Handoff to:** R9 (Manufacture a Resource Moat) — R8 addresses retention from the customer's side; R9 makes a critical resource scarce or inimitable so competitors cannot enter. (Supplier/partner leverage — the venture's dependency on external parties — is addressed later, at R10.)
...
**→ This skill: R8 — Create a Switching Cost** (F3, first requirement; begins the market position function)

**Design principle — architect for at-scale, not for launch.** Every requirement is designed for the venture operating at the scale required to pay back all investment capital at a competitive rate of return. The switching cost trajectory is assessed at commercial scale, not at launch cohort. An accumulation mechanism that is not material by month 12 at scale is not R8.

---

## R8 definition

> Design a mechanism that makes the product more valuable to the customer over time — and therefore harder to leave — as a structural property of the architecture, not as a contractual obligation. The switching cost must grow from genuine accumulated value, not from friction imposed on the customer.

F3 context: F1 established that the venture can create value surplus (the cost floor is below the value ceiling). F2 established that customers can adopt the product into their routine. F3 asks: can the venture hold its market position against competitors and substitutes once it has been established? R8 is the first F3 requirement — it addresses whether the value the venture creates is retained within the customer relationship or accessible to any competitor who enters later.

**The switching cost test:** a genuine switching cost grows from value accumulation, not from artificial lock-in. Contractual lock-in (minimum terms, exit penalties) is not R8. Any incumbent can impose these — they create customer resentment and do not build architectural advantage. A genuine switching cost makes the alternative worse by comparison, not harder to access.

---

## Why high satisfaction is not enough

Customer satisfaction, NPS, and brand loyalty are not R8. Customers can be highly satisfied and still leave for a comparable alternative when one appears — because nothing structural makes switching costly.

The test is not "do customers want to stay?" but "would switching require customers to give up something of genuine value they have accumulated?" Satisfaction makes switching undesirable. A structural switching cost makes switching economically irrational, regardless of satisfaction level.

**The market position problem:** once a venture has established product-market fit and activated a customer base, it faces a structural vulnerability. A well-resourced incumbent can observe the new BFF, replicate the key features, and compete for the established customer base. Without a switching cost, the venture's first-mover advantage is temporary. R8 is the mechanism that converts first-mover advantage into structural position.

---

## The three types of structural switching cost

### 1. Data accumulation

The product becomes more accurate, personalised, or effective as it learns the customer's patterns. Switching means starting over — losing the accumulated learning. The switching cost is the value of the data the customer would leave behind.

Examples: a financial planning tool that has learned a household's spending patterns over three years produces increasingly accurate projections — a new tool starts from zero; a health monitoring platform that has built a longitudinal baseline for an individual cannot be easily replicated by a new entrant; a demand forecasting tool that has ingested two years of a retailer's transaction data and learned seasonal patterns specific to their product mix.

**Diagnostic test:** does the product produce outputs that improve in quality or relevance over time, based on customer-specific data? If yes, and if the customer cannot export and transfer that learning to a competitor's product, the switching cost is data accumulation.

### 2. Integration depth

The product becomes embedded in the customer's workflow, tools, or relationships. Switching requires re-integrating every connected system. The switching cost is the re-integration effort.

Examples: an HR platform integrated with payroll, expense management, and compliance reporting — replacing it requires re-integrating every connected system; a procurement tool embedded in a business's supplier approval workflow — switching requires rebuilding the approval chain from scratch; an API-first product woven into a customer's software development pipeline.

**Diagnostic test:** how many external systems, workflows, or relationships depend on the product being in place? The switching cost is roughly proportional to the re-integration effort across all dependencies.

### 3. Relationship capital

The product builds trust, social proof, or network connections that belong to the customer's account. Switching means losing these. The switching cost is the relationship value accumulated.

Examples: a professional networking platform where the customer's reputation is built in that network — leaving means abandoning an audience they have cultivated; a marketplace where a seller's review history and buyer relationships live — switching platform means starting a reputation from zero; a platform where a customer's history of successful interactions creates verifiable trust signals that do not transfer.

**Diagnostic test:** does the product accumulate trust signals, reputation, or relationships on behalf of the customer that are specific to the platform and do not transfer? If yes, the switching cost is relationship capital.

---

## This skill IS a design loop

Running R8 is not a three-section form filled once. It is one turn of the IVE design loop — **Diagnose → Theorize → Productize → Simulate** — and the loop only closes when Simulate clears. The point of the loop is *learning by reasoning*: to reason this design to failure on paper and pivot **here, in simulation — not after launch, in market.** R8 is an F3 challenge; it still runs the Simulate gate. The cost floor updates with the accumulation infrastructure R8 adds, so the gate is live — do not skip it because "it's a lock-in challenge."

The four steps map onto this skill's existing structure (note the F3 ordering — resource → strategy → theory):
- **Diagnose** — SR1 Extended Product Use Routine / Key Store of Value (isolating what accumulates and can be made subject to loss).
...
- **Theorize** — the Lock-In Theory of Change (SR2); named mechanism, prescriptive not descriptive.
- **Productize** — the Lock-In Strategy (SR3) + the accumulation layer in the architecture + the CTM/AOM update (the design decision and its operational imprint).
...
A gate that cannot fail is theatre. Even with placeholder figures, the Simulate step must be able to fail — or the loop never fires and R8 has not been designed, only described.
...
- **Iteration cap.** If three loop-backs do not converge, stop and flag — do not spin. Write a `FLAG FOR TOM` block stating why R8 will not clear and what decision is needed.
...
- **Close criterion.** Mark R8 complete only when Simulate clears (real PASS) or a BORDERLINE is consciously accepted with its conditions recorded as critical assumptions. An unvalidated provisional pass closes the *design* but leaves the loop open pending data — record it that way.

---

## Process — step through each sub-requirement in order

Note: C8 uses a different ordering than C1–C7. In F3, the pattern is: identify the resource/store of value → state the Strategy → explain the Theory of Change. This is intentional — at F3, the design question starts with what value exists to be captured, then names the strategy, then grounds it in theory.

Open by displaying this checklist to the user:

```
R8 sub-requirements
  [ ] 1. Extended Product Use Routine / Key Store of Value
  [ ] 2. Lock-In Theory of Change
  [ ] 3. Lock-In Strategy
```
...
### Sub-requirement 1: Extended Product Use Routine / Key Store of Value

*The steps involved in using the interim offering and where they interface with the customer's life/organizational context (in terms of time, place, relationships, money, and identity) / Dimensions of the product's use best able to be leveraged to accrete value in the form of money/elimination of fear/elimination of stress that can be made subject to loss upon switching.*

This has two parts:

**(a) Extended Product Use Routine:** how does product use extend beyond a single transaction into the customer's ongoing life or operations? Where does the product interface with their identity, relationships, or workflows over time?

**(b) Key Store of Value:** from that extended routine, what specifically accumulates that can be made subject to loss? Apply the three types (data accumulation / integration depth / relationship capital) to identify the primary store.

**Critical point — customer ownership:** the accumulated asset must be experienced by the customer as theirs, not the venture's. Data that the venture collects and the customer cannot access or control is not a switching cost — it is a data extraction model.

Ask: how does product use extend into the customer's life over time, and what specifically accumulates that would be lost on switching?

---

### Sub-requirement 2: Lock-In Theory of Change

*The parameter on which the Key Store of Value rests, and the theory/ies most effective in explaining how to solve for that parameter.*

What mechanism causes the store of value to accumulate? What theory explains why customers experience this accumulation as valuable and experience its potential loss as costly?

Point to `/theory-of-change-custom` (invoke as "R8 Lock-In") for the formal ToC.

Ask: what is the parameter the store of value rests on, and what theory explains why customers will experience its potential loss as costly?

---

### Sub-requirement 3: Lock-In Strategy

*The best way to operationalize the Lock-In Theory of Change for accreting value that the customer risks losing upon switching to a competitor.*

⚠ **MANDATORY — before writing the Lock-In Strategy, complete these three steps in order:**

**Step 3a — Baseline honestly.** State where the current BFF stands on switching cost / accumulation. Not "not yet addressed" and not "already solved" — what does it actually do, and what does it leave unresolved?

**Step 3b — Generate ≥2 candidate BFF mutations.** Structural changes (not features) that could accumulate a more material switching cost than the baseline. If you cannot produce two genuine mutations, say so explicitly — that is justification mode. Do not paper over it. Do not proceed to Step 3c until two real mutations exist on the table.

**Step 3c — Evaluate and decide.** For each candidate: does it grow from genuine value accumulation (not artificial friction)? Does the customer experience the accumulated asset as theirs? What does it cost in coherence with R1–R7? Record the decision. Adopt a mutation (restate the evolved architecture) or hold the current one with explicit reasons each alternative was rejected. Never "it already works."

⛔ **REQUIRED OUTPUT CONTRACT — emit this block before writing the Lock-In Strategy. No strategy text is valid without it.**
...
Only after the contract block is emitted, write the Lock-In Strategy.

Must pass the switching cost test: does this grow from genuine value accumulation, not artificial friction? State as a strategy. Include the trajectory (when the switching cost becomes material).
...
**Critical constraint — consistency with R5.** The activation mechanism established in R5 is often the same mechanism that begins the accumulation. The Lock-in Strategy must be consistent with — and ideally build directly on — R5.

The elegance test: a strong Lock-in Strategy accumulates switching cost without the customer noticing — because the accumulation is a byproduct of using the product in the way it was designed to be used.

Note: C8 does NOT end with a Business Form Factor sub-requirement — F3 challenges do not have incremental BFF statements. The output ends with the Lock-In Strategy and the quality check.

Ask: how is this accumulation built into the product architecture? When does the switching cost become material?

---

## Productizing R8 — Strategy → architecture

The Lock-In Strategy names how value accumulates in the customer relationship. Productizing it means shaping the Working Product so accumulation is a structural by-product of normal use — not a deliberate action the customer takes.

**Target product component:** Working Product (Consuming — accumulation layer)

Work through these two questions before finalising the Lock-In Strategy:

**1. Invisible accumulation.** Does the switching cost build automatically as the customer uses the product as designed — without requiring them to take a deliberate step? Accumulation that requires opt-in, registration, or active engagement has dropout. Structural accumulation does not. The best R8 solutions make the switching cost a consequence of the product doing its job, not a feature the customer chooses.

**2. R5 continuity.** Is the accumulation mechanism a direct extension of the activation mechanism R5 established? The use routine that removes the learning disruption should be the same routine that begins building the switching cost. If R8 requires a different behaviour from the customer than R5 already produces, the architecture is asking too much of the customer transition. Confirm the two are structurally continuous.
...
F3 requirements do not produce cumulative BFF statements. The productizing step here is complete when the accumulation mechanism is confirmed as structurally embedded — not as a separate loyalty feature.

⛔ **MANDATORY HANDOFF — LOCK-IN STRATEGY CONFIRMED. DO NOT EDIT THE VDR. DO NOT PROCEED TO R9.** The next and only permitted action is Section 5 (CTM Update). Announce explicitly: "Lock-In Strategy confirmed. Running Sections 5–8 now before any VDR editing." Then proceed directly to Section 5.
...
```
SUMMARY
  Lock-In Strategy: [strategy in one sentence — the architectural direction]
  Store of value: [what accumulates and why it is genuine, not artificial friction]
  Grounded in: [named theory]
  Switching cost becomes material: [timeframe / trigger point]

R8 — SWITCHING COST (LOCK-IN)
---
EXTENDED PRODUCT USE ROUTINE / KEY STORE OF VALUE
  Extended routine: [how product use extends beyond a single transaction]
  Store of value type: [Data accumulation / Integration depth / Relationship capital]
  What accumulates: [specifically — not generically]
  Switching cost trajectory: [when it becomes material]

LOCK-IN THEORY OF CHANGE
  Parameter: [what the store of value rests on]
  Theory: [citation]
  Context: [...]
  Outcome: [...]
...
LOCK-IN STRATEGY
  [answer]
  Switching cost test: [genuine accumulation or artificial friction?]
  R5 interaction: [does activation and accumulation share the same mechanism?]
  R10 interaction: [how does this contribute to the competitive moat?]
...
QUALITY CHECK
  Does the switching cost grow from genuine value accumulation? [Yes / No]
  Could an incumbent replicate this without changing their BFF? [Yes — rethink / No — good]
  Is the trajectory material within a commercially relevant timeframe? [Yes / No]
  Does the customer experience the accumulated asset as theirs? [Yes / No — if No, redesign]
  Confidence: [High / Medium / Low]
```

---

## Common failure patterns at R8

| Failure | What it looks like | What it means |
|---------|-------------------|---------------|
| Contractual lock-in as R8 | "Customers sign 24-month contracts with exit penalties" | Minimum terms and exit penalties are not switching costs. They reduce churn but do not build value and create resentment. Any incumbent can impose these — they create no architectural advantage. |
| Switching cost without accumulation mechanism | "Customers won't leave because we're best in class" | The mechanism that creates accumulated value must be specified. Brand preference is not a switching cost — it does not make switching economically irrational. |
| Accumulation that benefits only the venture | "We build a rich data profile on each customer" | If the data or relationship capital accumulated belongs to the venture and not the customer, customers will resist it rather than embrace it. The switching cost must grow from value the customer experiences as theirs. |
| Confusing retention with lock-in | "Our NPS is 72 — customers love us" | High NPS and customer satisfaction are not R8. Customers can have high satisfaction and still leave for a comparable alternative when one appears. R8 creates structural impediment to switching, not emotional preference. |
| Immaterial trajectory | "The switching cost builds over 5–7 years" | A switching cost that is not material within the competitive threat horizon does not protect the venture's position. If a competitor can enter and replicate within the tolerance window, R8 is not functioning. |
...
```
R7 (Gateway Partners) → R8 (Switching Cost) → R9 (Supplier Leverage) → R10 (Competitive Moat)
```

R8 begins F3. It follows the completion of the full F2 architecture (R4–R7) and depends directly on the activation mechanism established in R5 — the use pattern that begins accumulation is usually the same pattern that R5 designed.

**R8 vs R10 distinction:** R8 addresses the customer relationship — it makes switching costly from the customer's side. R10 addresses the market position — it makes competitive entry costly from the competitor's side. Both together constitute the structural defence of the venture's market position. R8 does not render R10 unnecessary; a switching cost without a competitive moat can be undermined if a new entrant offers enough value to overcome the switching cost. Design both.

**Cross-requirement elegance:** the strongest R8 solutions accumulate switching cost as an automatic byproduct of R5 activation. The customer adopts the routine (R5), and in doing so begins accumulating the asset (R8) — without any additional action required. This is both architecturally elegant and practically hard to copy: the accumulation is inseparable from the use, not a feature added on top.
...
This challenge's job is **not** to confirm the current BFF already handles switching cost / lock-in. It is to find out whether changing the BFF could handle it **materially better**. Treat the current BFF as the baseline to beat — not the answer to defend.

**VA-3 — generate before you justify.** Before writing this challenge's Strategy:
1. **Baseline, honestly.** Where does the current BFF stand on switching cost / lock-in? Not zero, not solved.
2. **Generative target.** "How could we change the BFF to do materially better on switching cost / lock-in than the baseline?"
...
4. **Evaluate each against the whole** — gain on switching cost / lock-in vs cost to coherence (VA-1) and to dimensions earned in earlier challenges.
...
- Edit `.claude/skills/balm-challenge-8-custom/SKILL.md` to modify
- **Skill chain (predecessor):** `/balm-challenge-7-custom` (R7) → `/balm-challenge-8-custom` (R8)
- **Skill chain (successor):** `/balm-challenge-8-custom` (R8) → `/balm-challenge-9-custom` (R9)
- After R8, update the CTM and AOM via Sections 5–6 of this skill before proceeding to R9 (format specs: `shared/ctm-diagram-spec.md`, `shared/aom-diagram-spec.md`; `/ive-ctm-custom` and `/ive-aom-custom` are deprecated — do not invoke)
...
These sections run after the BALM diagnostic work above is confirmed. They make R8 self-contained: it updates the CTM and AOM, updates the financial model, and runs the FIT gate before marking R8 complete.
...
|-------|-------------------|---------|
| CTM | `[venture]-ctm-at-C[N].html` | `acme-ctm-at-C8.html` |
| AOM | `[venture]-aom-at-C[N].html` | `acme-aom-at-C8.html` |
| Fin-Sim | `[venture]-fin-sim-at-C[N].html` | `acme-fin-sim-at-C8.html` |
...
Ask the user:
- "What product component does the Lock-In Strategy add or change?"
- Which CTM phase does it affect? **Phase 2 — Consuming (accumulation) + Phase 4 — Repeating**
- Which 4-D product delivers it? **Working Product**
- What is the specific element? (e.g. "a personalisation layer that accumulates customer-specific data with each use and degrades in value if transferred")
- What state change does it produce in the customer? (Cognitive / Emotive / Behavioural)

**R8 — update mode:** Add or modify only the affected Phase 2 (accumulation) and Phase 4 (Repeating) rows in the existing CTM. Do not rebuild the whole document.

**Catch-up mode:** If the CTM does not yet exist (because prior challenges were run without this integrated structure), build it retrospectively from all confirmed strategies for R1 through R7, then add R8's accumulation mechanism.
...
- What role enables the accumulation mechanism (if any operational activity is required)?
- How long does it take per unit (per use session that accrues value)?
- What is the volume driver? (per use session / per customer cohort / fixed per LMU)

**Check for common errors before writing:**
- Has a pre-purchase activity been divided by the conversion rate?
- Has a new enabler role been added if the accumulation mechanism requires QA or data management?
- Has a new HQ function been added if data accumulation requires central infrastructure?
...
**R8 — update mode:** Add or modify only the affected Using stream activity rows. Do not rebuild the whole AOM. Note: if accumulation is fully automated (no operational activity required), the AOM impact may be infrastructure cost only at HQ level — confirm this explicitly.

**Catch-up mode:** If the AOM does not yet exist, build it retrospectively from all confirmed strategies for completed challenges before updating for R8.
...
1. Map those activities to the financial model's cost lines:
   - **Using stream at LMU level** → drives COGS (accumulation activities per use session)
   - **Using/Making streams at HQ level** → drives fixed overheads (data infrastructure, accumulation systems)
...
⚠ **FMOS uses the cost denominator: `FMOS = (ceiling − floor) ÷ floor` = `(WTP_low − Cost_high) / Cost_high`** (Simanis, *Built to Hold*, p.7). An earlier version of this section divided by the ceiling — `(ceiling − floor) ÷ ceiling` — which is the **wrong (price) denominator**: that is the gross-margin metric, a different and incompatible figure that understates FMOS. Divide by the **floor**. WTP_low = R2 KMC / price ceiling; Cost_high = AOM high-point unit cost.

**For R8:** Use the R2 KMC as the price ceiling. The cost floor updates with the new AOM additions from R8 (accumulation infrastructure and operational activities).
...
- **FAIL (<15%):** Return to Section 2 (accumulation mechanism diagnosis). State which lever is off: cost floor too high, or price ceiling too low.

**Theory robustness check (embedded in FIT):**
Ask: "What is the named theory grounding the Lock-In Strategy? Name the mechanism and the precedent."
...
- Mark **R8 complete** (or fail) on the status board
- Route to **R9** (Manufacture a Resource Moat)
